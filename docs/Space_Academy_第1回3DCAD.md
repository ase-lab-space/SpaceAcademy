...
本記事では、3D CADソフト「Fusion 360」の基本操作を体験し、実際に手のひらサイズの飛行機を設計することを目標とします。
これは機械系ハンズオンの第1回にあたる資料です。今回作った機体は、次章 [02_構造解析をやってみよう.md](./02_構造解析をやってみよう.md) で、力をかけて壊す（強度をテストする）ための題材として使います。
...
Fusion 360 はその中でも、無料で使えて情報も多く、設計からシミュレーションまで一通りこなせるため、最初の1本として非常に扱いやすいソフトです。
...
この関係を対応させると、次のようになります。

| 飛行機の翼 | 腕を横に伸ばした状態 |
| --- | --- |
| 翼の先端 | 手先 |
| 翼にかかる空気の力・荷重 | 手に持ったカバン |
| 翼のつけ根（胴体との接続部） | 肩のつけ根 |
| 一番折れやすい場所 | 一番つらい肩 |

次章の構造解析では、今日作る機体の胴体を固定して翼の先を押し、「どこが一番折れそうか」を虹色のヒートマップで観察します。狙い通り、翼のつけ根が真っ赤に光るはずです。~~構造屋は物が壊れる瞬間が好きなので、来週をお楽しみに。~~

## 準備するもの

- [Fusion 360](https://www.autodesk.com/products/fusion-360/personal)（学生は**無料のEducationライセンス**で使えます。要学生確認）
...
[![完成イメージ](./91-images/airplane_complete.png)](./91-images/airplane_complete.png)

> ※ 本記事中の画像パス（`./91-images/...`）は仮置きです。当日のスクリーンショットを撮って差し替えてください。
...
操作を始める前に、この3つのショートカットだけ覚えておくと安心です。

- 元に戻す：`Ctrl + Z`（失敗しても戻せるので、怖がらず試してよい）
- 全体表示：画面下のナビゲーションバーの **Fit**（迷子になったらこれで戻る）
...
[![胴体の押し出し](./91-images/01_fuselage.png)](./91-images/01_fuselage.png)
...
[![主翼の作成](./91-images/02_wing.png)](./91-images/02_wing.png)
...
| どの平面を選べばいいか分からない | 選択肢が多い | 毎回、同じ底の平面を選ぶと決めておく |
...
- <https://help.autodesk.com/view/fusion360/JPN/>（Fusion 360 公式ヘルプ・チュートリアル）
