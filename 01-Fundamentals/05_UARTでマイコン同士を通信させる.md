...
2. Thonnyの **ファイル → 名前を付けて保存 → Raspberry Pi Pico** から、ファイル名を **`main.py`** として保存する(`main.py`という名前で保存すると、その後は**PCにつながなくても、電源を入れるだけで自動的にそのプログラムが実行される**ようになります)
    ...
    ![Thonnyのファイルメニューから「名前を付けて保存」を選ぶ](images/class5/05a_thonny_menu_save_as.png)

    ![保存先を選ぶダイアログで「Raspberry Pi Pico」を選ぶ](images/class5/05b_thonny_save_dialog_target.png)

    ![ファイル名を main.py にして保存する](images/class5/05c_thonny_save_as_mainpy.png)
...
> 💡 `try`〜`except UnicodeError`で囲んでいるのには理由があります。詳しくは5章で説明しますが、受け取ったデータが文字として解釈できないとき(`decode("utf-8")`が失敗するとき)に、プログラムが止まってしまうのを防ぐためのものです。今はそのまま使ってください。
...
![ボードBのThonnyシェルに受信ログが流れている様子。左上の[main.py]タブがボードAに書き込んだ自走用プログラム、左下の赤枠がボードBで実際に受信できているログ](images/class5/07_uart_send_receive_result.png)
...
> ⚠️ **緑色の▶(実行)ボタンを押した直後にエラーが出た場合**
> これは今の説明(ボーレートの不一致)とは**別の原因**です。直前まで9600bpsのプログラムを実行し続けていたボードに対して、コードを書き換えてすぐ▶を押すと、**前回のプログラムが使っていたUART・GPIOのピンが、まだ手放されていない状態**でぶつかってしまうことがあります(MicroPythonは、プログラムを止めただけではピンの占有状態を自動で解放しません)。
>
> 対処法は簡単です。**▶(実行)を押す前に、赤い■(停止)ボタンを押すか、シェルの中で`Ctrl+D`を押してソフトリセットしてください。** これでピンの状態がまっさらに戻り、新しいボーレートで正しく実行できます。ハードウェアの設定(ピン・ボーレートなど)を変えてコードを実行し直すときは、**「止める→やり直す」を癖にする**とこの手のエラーを避けられます(第1回の「[3.6 モーターを止めたあと、PCと繋がらなくなったら](./03_マイコン入門とLチカとモーター.md)」も同じ原因です)。

ボードAは9600bpsのまま送信を続けてください。無事に実行できると、ボードBのシェルには、意味の分からない記号や文字化けした文字列が表示されることもあれば、**`UnicodeError`というエラーが出てプログラムが止まってしまう**こともあります。どちらが起きても正常です(4800と115200の両方を試すと、違いが観察できるかもしれません)。

*(ここにボーレート不一致で文字化けした受信結果、またはUnicodeErrorが表示された画面のスクリーンショットを貼る。`08_baudrate_mismatch_garbled.png`)*

これは、受信側が「1bitの長さ」を送信側と違う長さで解釈してしまい、区切る位置がズレてしまうために起こります。ズレた結果、意味のないバイト列を受け取ることになりますが、**そのバイト列がたまたま「文字として解釈できる形」でなかった場合、`decode("utf-8")`がエラーを起こします**(これが受信側コードで`try`〜`except`を使っていた理由です)。この`except`が無いと、文字化けどころかプログラムそのものが止まってしまいます。ボードBのボーレートを`9600`に戻し、正しく表示されることを確認してください。
...
> 💡 モーターを止めたあとPCと繋がらなくなった場合は、[03_マイコン入門とLチカとモーター.md](./03_マイコン入門とLチカとモーター.md)の「3.6 モーターを止めたあと、PCと繋がらなくなったら」を参照してください。

## 8. 次回への接続

今日はUARTだけを扱いましたが、マイコンとセンサをつなぐときには、**I2C**や**SPI**という別の通信方式もよく使われます。どちらもクロック線を使う**同期式通信**で、今日の「クロックが無い」UARTとは考え方が異なります。

ただし、センサを使ったプログラムでは`import`という形で**ライブラリ**(誰かが作ってくれた関数のまとまり)を使うことになります。次回はセンサの前に、そもそも**関数とは何か・ライブラリとは何か**を扱い、次々回で実際にセンサ(I2C/SPI)をつないで体験します。
