# 電子工作 第5回 I2CとSPIでセンサと話す

前回([06_関数定義とライブラリ.md](./06_関数定義とライブラリ.md))は、**関数とライブラリ**の使い方を学びました。今日はその知識を武器に、初めて**マイコンとセンサ**を会話させます。相手が変わるだけでなく、会話の仕方(プロトコル)も新しく2つ、**SPI**と**I2C**を学びます。

![今日のゴール](images/class7/00_lesson_thumbnail.svg)

> **今回のマインドセット**
> - 同じセンサに、まずSPIで、次にI2Cで話しかけます。「相手は同じなのに、話し方が2通りある」ことを体感してください
> - 前回学んだ通り、`import bme280`は「誰かが作ってくれた関数の集まりを持ってくる」行為です。今日はその中身(レジスタを読み書きする関数)がどうなっているかも覗いていきます
> - センサとの会話は、これまでの「0か1か」より一段階複雑です。難しく感じたら、いったん手を動かすことを優先しましょう
...
![BME280モジュール](images/class7/01_bme280_photo.jpg)
...
![SPI配線図](images/class7/02_spi_wiring.svg)
...
MicroPython標準にはBME280用のライブラリが含まれていないため、[`sample-code/bme280.py`](./sample-code/bme280.py)を追加します。このファイルはSPI・I2C両対応の教育向けドライバとして、SpaceAcademyが独自に用意したものです。前回の「他人のライブラリを使う」の実践と同じ要領で、手元に持ってきましょう。
...
![シェルに気温・気圧・湿度のタプルが1秒おきに表示されている様子(赤枠部分)](images/class7/07_spi_result.png)
...
![SPIのトポロジー(マスター⇔センサの4本の線)](images/class7/12_spi_topology_ref.svg)
...
![SPIの通信イメージ(タイミング)](images/class7/03_spi_protocol.svg)
...
前々回学んだUARTとの大きな違いは、**クロック線(SCK)がある**ことです。UARTは「スタートビット+事前に決めた速度」で足並みを揃えましたが、SPIは**クロックが刻まれるたびに1bitずつ、送信と受信を同時に**行います(全二重)。クロックがある分、UARTよりも高速かつ正確に通信できます。
...
![BME280公式データシート(Table 18: Memory map)](images/class7/10_bme280_datasheet_memorymap.png)
...
![ビットシフトによる気温データの取得(3つのレジスタを1列に並べる)](images/class7/11_bit_shift_assembly.svg)
...
![I2C配線図](images/class7/05_i2c_wiring.jpg)
...
![I2Cのトポロジー(2本の線に複数の機器がぶら下がる)](images/class7/13_i2c_topology_ref.png)
...
![I2Cの通信の流れ(アドレス指定とACK)](images/class7/06_i2c_protocol.svg)
...
| 今日の学び | クロックがあるので前々回のUARTより速く安定 | 配線は少ないが、アドレスとACKで相手を確認する手間がある |
...
![Plotterのグラフ(右、細かく揺れている)と、平均値・分散・標準偏差の出力結果(左、赤枠部分)](images/class7/14_sensor_noise_stats.png)
