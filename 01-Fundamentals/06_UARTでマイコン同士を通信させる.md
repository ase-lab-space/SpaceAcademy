...
ただし、この仕組みには弱点もあります。送信側と受信側の内部クロック(時間を刻む部品)は、**完全に同じ速さとは限りません**。ごくわずかな速度差でも、1フレームの中で何度もビットを数えるうちに、少しずつ誤差が積み重なっていきます。

![クロックのズレと中央サンプリングの工夫](images/class5/05_clock_drift_and_midbit_sampling.svg)

もし受信側が**ビットの境界(電圧が切り替わる瞬間)ぴったり**を読み取ろうとしていたら、この誤差によって隣のビットを誤って読んでしまう危険があります。そこでUARTの受信回路は、境界ではなく**各ビットの「真ん中」付近で電圧を読み取る**という工夫をしています。ビットの中央は電圧がもっとも安定している区間なので、多少のタイミングのズレがあっても、正しい値を読み取れる余裕(マージン)が生まれます。
...
> 2. とはいえ、送信側と受信側の内部クロックは、完全に同じ速さとは限りません。もし少しズレていたら、フレームの後半でどんな問題が起きるでしょうか。また、UARTがそれでも安定して読み取れているとしたら、どんな工夫をしていると思いますか。今日見た波形図やサンプリングの図を見返しながら考えてきてください。
>
> 次回、みんなで答え合わせをします。
