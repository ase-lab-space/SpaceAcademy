...
![CADによる設計例(左)とシミュレーション例(右)のイメージ図](images/class1/image-11.png)
...
![腕を伸ばしてカバンを持つ様子と、飛行機の翼にかかる力の対応イメージ](images/class1/image-12.png)
...
> ⌨️ **キーボードの配置に自信が無い人へ**
> ショートカットキーは、キーボード上の位置が分からなくても大丈夫です。[Autodesk公式のショートカット一覧(X/Twitter)](https://x.com/Fusion360Japan/status/1267708099294453761)のような画像を手元に置いておくと、押すべきキーを目で探せます。今日使うのは上の3つだけなので、無理に覚える必要はありません。
...
この2つは、②の「作成」メニューの中にあります(フィレットは「修正」メニューの中です)。

> 📐 四角形を描くコマンドには何種類かありますが、今日は**すべて「中心矩形」を使います**。「作成」メニューの「長方形」の中から選べます(中心点から左右対称に広がる四角形が描けるので、今回のように中心線に合わせて配置したい場合に便利です)。

![「作成」メニューの中の①スケッチを作成、②押し出し](images/class1/image-19.png)

> 🎥 **もっと詳しく知りたい人へ**
> 文章だけで分かりにくい操作があれば、YouTubeで「Fusion 360 スケッチ 初心者」のように検索すると、実際の画面の動きを見られるチュートリアル動画がたくさん見つかります。困ったら検索してみてください。
...
> 💡 **ワンポイント:スケッチする平面について**
> ここからは「胴体の面」の上に新しいスケッチを作っていきます。実は、モデルの**面を直接選んでスケッチを作る**と、後で胴体の形を直したときにスケッチの位置がズレたり、エラーが起きたりすることがあります(これを「依存関係」と呼びます)。
> 今日は気にせず面を選んで進めてもらって大丈夫ですが、もし操作に慣れてきたら、**「構築」→「オフセット平面」**で胴体から独立した"補助平面"を作り、そこにスケッチする方法もあります。形を作り直す予定がある場合に安心な、少し発展的なテクニックです。
